...
class Model_projects extends CI_model{
    function getprojects($params = array()){
        $this->db->select('*');
        ...
        //related product
        if(!empty($params['projects_id'])){
            $this->db->where('id_projects !=', $params['projects_id']);
        }
        //set start and limit
    ...
    function upload_data(){
        $this->reset_db_projects();
        //$this->db->query("TRUNCATE TABLE projects");
        //$this->db->query("TRUNCATE TABLE projects_tagmap");
        //$this->db->query("TRUNCATE TABLE projects_tags");
            ...
            if ($media['file_ext'] == '.csv') {
                ...
                foreach($file_data as $row)
                {
                    ...
                    if ($ssrc != '') {
                        ...
                        $targetpath = "../asset/foto_projects/".$ssrcimg;
                        //upload gambar
                        /*
                        unset($config);
                        //icon
                        $config['upload_path'] = '../asset/foto_projects/';
                    ...
                    "judul"         => $row["Title"],
                    "slug"          => $row["Alias"],
                    "isi_projects"    => $row["Introtext"].$row["Fulltext"],
                    "gambar"        => $this->db->escape_str($ssrcimg),//$rowData[0][20],
                    "projects_views"  => $row["Hits"],
                    ...
                    if ($this->db->insert("projects",$data)) {
                        # code...
                        $data['error'] = FALSE;
                        $slugprojects = $row["Alias"];//$this->slug->create_uri($this->input->post('judul'));
                        //toxy categories
                        //$this->load->model('Tags_model','tags');
                        $datpost = $this->get_projects_id($slugprojects);
                        $idpost = $datpost[0]['id_projects'];
                        $tags = $row["Tags"];//$this->db->escape_str($this->input->post('tagshasil'));
                        if ($tags != '') {
                            $this->model_projects_tags->insert_projects_tagsdb($tags);
                            $this->model_projects_tags->insert_projects_tagsmap($tags,$idpost,'projects');
                        }
                        $Category = $row["Category Name"];
                        if ($Category != '') {
                            $this->model_projects_categories->insert_projects_categoriesdb($Category);
                            //$this->model_projects_categories->insert_projects_categoriesmap($Category,$idpost,'projects'); //multi cat
                            $this->model_projects_categories->insert_projects_categoriesid($Category,$idpost,'projects'); //single cat
                        }
                        //toxy categories
                        ...
                        //$this->categories_model->insert_categoriesmap($cats,$idpost,'projects');
                    }
                ...
                for ($row = 2; $row <= $highestRow; $row++){  
                    ...
                    $data = array(
                        //"id_projects"     => $rowData[0][0],
                        "username"      => 'admin',
                        "judul"         => $rowData[0][2],
                        "slug"          => $rowData[0][3],
                        "isi_projects"    => $content,
                        "gambar"        => $this->db->escape_str($ssrcimg),//$rowData[0][20],
                        "projects_views"  => $rowData[0][14],
                    ...
                    if ($this->db->insert("projects",$data)) {
                        # code...
                        $data['error'] = FALSE;
                        $slugprojects = $rowData[0][3];//$this->slug->create_uri($this->input->post('judul'));
                        //toxy categories
                        //$this->load->model('Tags_model','tags');
                        $datpost = $this->get_projects_id($slugprojects);
                        $idpost = $datpost[0]['id_projects'];
    ...
    function reset_db_projects(){
        $this->db->truncate('projects');
        $this->db->truncate('projects_tagmap');
        $this->db->truncate('projects_tags');
    ...
    function kategori_projects_edit($id){
        return $this->db->query("SELECT * FROM kategori where id_kategori='$id'");
    }

    function kategori_projects_update(){
        $datadb = array('nama_kategori'=>$this->db->escape_str($this->input->post('a')),
                        'kategori_seo'=>seo_title($this->input->post('a')),
                        'aktif'=>$this->db->escape_str($this->input->post('b')));
    ...
    function kategori_projects_delete($id){
        return $this->db->query("DELETE FROM kategori where id_kategori='$id'");
    }

    //categories
    function categories_projects(){
        return $this->db->query("SELECT * FROM categories_lists where group_id=7 ORDER BY id DESC");
    }
    //
    function sensorkata(){
    ...
    //toxi tag
    function tags_projects($post_id){
        //return $this->db->query("SELECT * FROM tag ORDER BY id_tag DESC");
        //$hasil = $this->db->query("SELECT t.slug FROM tagmap bt, projects b, tags t WHERE bt.tags_id = t.tags_id AND b.id_projects = '$post_id' GROUP BY t.tags_id");
        //$tagsdata = implode(',',$hasil);
        $hasil = $this->db->query("SELECT t.slug
                                    FROM projects_tagmap bt, projects_tags t, projects b
                                    WHERE bt.id_projects = '$post_id'
                                    AND t.tags_id = bt.tags_id
                                    GROUP BY t.tags_id");
        return $hasil;
    }
    function get_projects_id($slug){
        return $this->db->query("SELECT * FROM projects where slug='$slug'")->result_array();
    }
    //toxi tag end
    function tag_projects_tambah(){
    ...
    function tag_projects_edit($id){
        return $this->db->query("SELECT * FROM tag where id_tag='$id'");
    }

    function tag_projects_update(){
        $datadb = array('nama_tag'=>$this->db->escape_str($this->input->post('a')),
                        'tag_seo'=>seo_title($this->input->post('a')));
    ...
    function tag_projects_delete($id){
        return $this->db->query("DELETE FROM tag where id_tag='$id'");
    }

    function komentar_projects($id_projects){
        return $this->db->query("SELECT * FROM komentar where id_projects = '$id_projects' AND aktif='Y'");
    }

    function kirim_komentar(){
        $datadb = array('id_projects'=>cetak($this->input->post('a')),
    ...
    /*
    function list_projects_rss(){
        return $this->db->query("SELECT a.*, b.nama_kategori FROM projects a LEFT JOIN kategori b ON a.id_kategori=b.id_kategori ORDER BY a.id_projects DESC LIMIT 10");
    }

    function list_projects(){
        return $this->db->query("SELECT a.*, b.nama_kategori FROM projects a LEFT JOIN kategori b ON a.id_kategori=b.id_kategori ORDER BY a.id_projects DESC");
    }
    */
    function list_projects_rss(){
        return $this->db->query("SELECT a.*, b.name FROM projects a LEFT JOIN categories_lists b ON a.id_categories=b.id ORDER BY a.id_projects DESC LIMIT 10");
    }

    function list_projects(){
        return $this->db->query("SELECT a.*, b.name FROM projects a LEFT JOIN categories_lists b ON a.id_categories=b.id ORDER BY a.id_projects DESC");
    }

    
    function list_projects_tambah(){
            $config['upload_path'] = '../asset/foto_projects/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            ...
            $this->load->library('slug', $config);
            $slugprojects = $this->slug->create_uri($this->input->post('judul'));
            // create the slug
            //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
            //fikar z
            ...
            if ($_FILES["gambar"]['name']==''){
                    $datadb = array('judul'=>$this->db->escape_str($this->input->post('judul')),
                                    ...
                                    'isi_projects'=>$this->input->post('isi_projects'),
        ...
        if ($this->db->insert('projects',$datadb)){
            //if(file_exists('../asset/foto_projects/'.$hasil['file_name'])){
                thumb('../asset/foto_projects/'.$hasil['file_name'],'400','200');
            //}
        }
        //fikar tag toxi
        //$ci=& get_instance();
    ...
    function list_projects_edit($id){
        return $this->db->query("SELECT * FROM projects where id_projects='$id'");
    }

    function list_projects_update(){
        $config['upload_path'] = '../asset/foto_projects/';
        $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
        $config['max_size'] = '3000'; // kb
        $new_name = substr(md5(uniqid(mt_rand())),0,10).'_'.time().'_'.$_FILES["gambar"]['name'];
        ...
        //fikar
            ...
            //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
            //fikar z
            $page = $this->db->where('id_projects', $this->input->post('id'))->limit(1)->get('projects')->row();
            ...
            if ($_FILES["gambar"]['name']==''){
                ...
                getnameimg('../asset/foto_projects/'.$page->gambar,'foto_projects');
                //unlink('../asset/foto_projects/'.$page->gambar);
            }
        ...
        $this->db->where('id_projects',$this->input->post('id'));
        
        if ($this->db->update('projects',$datadb)){
            if ($imgupdate == true){
                //if(file_exists('../asset/foto_projects/'.$hasil['file_name'])){
                    thumb('../asset/foto_projects/'.$hasil['file_name'],'400','200');
                //}
    ...
    function list_projects_delete($id){
        $page = $this->db->where('id_projects', $id)->limit(1)->get('projects')->row();
        //print_r($page);
        getnameimg('../asset/foto_projects/'.$page->gambar,'foto_projects');
        return $this->db->query("DELETE FROM projects where id_projects='$id'");
    }
    ...
    function update_projects($data = array()){
        ...
        if (is_array($data) && ! empty($data))
        {
            ...
            $this->db->update_batch('projects', $data, 'id_projects');
        }
    }
}