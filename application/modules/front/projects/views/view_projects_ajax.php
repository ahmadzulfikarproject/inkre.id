		        ...
		        <div class="post-list col-xs-12 " id="enquiryList">
		        	...
		        	<div id="projects-list" class="row">
			        	<?php if(!empty($posts)): $no = 1; foreach($posts as $post): ?>
			                <?php $isi_projects = strip_tags($post['isi_projects']); 
	                        ...
	                        if ($post['gambar'] == ''){
	                        	...
	                        	if (!file_exists("asset/foto_projects/".$foto)) {
	                        		if (file_exists("asset/foto_projects/".$post['gambar'])) {
	                        			$foto = $post['gambar'];
	                        		}
	                        		else{
	                        			$foto = 'nophoto.jpg';
	                        ...
	                        ?>
			                <div class="col-sm-3 col-sm-heightz col-md-heightz col-lg-heightz col-topz item-list">
	                            <div class="responsive-container box-item schfx" itemscope>

	                                <div class="dummy50"></div>
	                                <a href="<?php echo base_url()."projects/detail/".$post['slug']; ?>" class="img-container" data-toggle="tooltip" data-placement="top" title="<?php echo $post['judul'] ?>" style="background-image: url('<?php echo base_url()."asset/foto_projects/".$foto ;?>');">
	                                    <div class="centerer"></div>
	                                    <img alt="<?php echo $post['judul'] ?>" class='img-thumbnailz hidden' src='<?php echo base_url()."asset/foto_projects/".$foto ;?>' class='img-responsive'>                           
	                                </a><!--img-container-->
	                                <div class="box-item-caption hidden">
	                                    ...
	                                    <h3 class="carousel-caption-headerz"><a href='<?php echo base_url()."projects/detail/".$post['slug']; ?>'><?php echo $post['judul'] ?></a></h3>
	                                </div><!--kotak-item-caption-->

	                            </div><!--responsive-container-->
	                            <div class="kotak-item-title text-center">
	                                <div class="box-title"><h3><a href='<?php echo base_url()."projects/detail/".$post['slug']; ?>'><?php echo $post['judul'] ?></a></h3></div>
	                                <span class="box-price"><?php echo $post['lokasi'] ?></span>
	                            </div>
	                        </div><!--col-->
	                        <?php if ($no % 4 == 0): ?>
		...
		function searchFilter(page_num) {
		    ...
		    $.ajax({
		        type: 'POST',
		        url: '<?php echo base_url(); ?>projects/ajaxPaginationData/'+page_num,
		        ...
		        beforeSend: function () {
		        	...
		        	//$js('#projects-list .item-list').fadeOut("fast");
		        ...
		        success: function (html) {
		            ...
		            $js('.loading').fadeOut( "fast", function() {
                        ...
                        //$js('#projects-list .item-list').hide().fadeIn();
