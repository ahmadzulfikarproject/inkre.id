...
class Contact extends MY_Controller
{
	...
	public function index($slug = '')
	{
		...
		$dat = $this->model_contact->contact(1)->row_array();
		if (empty($dat)) {
			redirect('main');
		}
		// print_r($id);
		// echo $dat['slug'];
		// $slug = $dat['slug'];
		// $ids = $this->db->where('id_contact', '1');
		...
		if ( ! is_dir('./captcha/'))
        {
            ...
            if ( ! mkdir ('./captcha/', 0755, TRUE))
            {
                $this->set_error('upload_no_filepath', 'error');
                return FALSE;
            }
        }
        if ( ! is_really_writable('./captcha/'))
        {
            // EDIT: change directory mode
            if ( ! chmod('./captcha/', 0755))
            {
		...
		$this->template->meta->add('keywords', $dat['meta_keywords']);
		...
		$this->breadcrumbs->push($dat['nama'], $dat['slug']);
