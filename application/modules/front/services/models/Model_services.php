...
class Model_services extends CI_model{
    function getservices($params = array()){
        ...
        //related product
        if(!empty($params['services_id'])){
            $this->db->where('id_services !=', $params['services_id']);
        }
        //set start and limit
            ...
            if ($media['file_ext'] == '.csv') {
                ...
                foreach($file_data as $row)
                {
                    ...
                    if ($ssrc != '') {
                        ...
                        $targetpath = "../asset/foto_services/".$ssrcimg;
                        //upload gambar
                        /*
                        unset($config);
                        //icon
                    ...
                    "judul"         => $row["Title"],
                    "slug"          => $row["Alias"],
                    "isi_services"    => $row["Introtext"].$row["Fulltext"],
                    "gambar"        => $this->db->escape_str($ssrcimg),//$rowData[0][20],
                    "services_views"  => $row["Hits"],
                    ...
                    if ($this->db->insert("services",$data)) {
                        # code...
                        $data['error'] = FALSE;
                        $slugservices = $row["Alias"];//$this->slug->create_uri($this->input->post('judul'));
                        //toxy categories
                        //$this->load->model('Tags_model','tags');
                        $datpost = $this->get_services_id($slugservices);
                        $idpost = $datpost[0]['id_services'];
                        $tags = $row["Tags"];//$this->db->escape_str($this->input->post('tagshasil'));
                        if ($tags != '') {
                            $this->model_services_tags->insert_services_tagsdb($tags);
                            $this->model_services_tags->insert_services_tagsmap($tags,$idpost,'services');
                        }
                        $Category = $row["Category Name"];
                        if ($Category != '') {
                            $this->model_services_categories->insert_services_categoriesdb($Category);
                            //$this->model_services_categories->insert_services_categoriesmap($Category,$idpost,'services'); //multi cat
                            $this->model_services_categories->insert_services_categoriesid($Category,$idpost,'services'); //single cat
                        }
                        //toxy categories
                        ...
                        //$this->categories_model->insert_categoriesmap($cats,$idpost,'services');
                    }
                ...
                for ($row = 2; $row <= $highestRow; $row++){  
                    ...
                    $data = array(
                        //"id_services"     => $rowData[0][0],
                        "username"      => 'admin',
                        "judul"         => $rowData[0][2],
                        "slug"          => $rowData[0][3],
                        "isi_services"    => $content,
                        "gambar"        => $this->db->escape_str($ssrcimg),//$rowData[0][20],
                        "services_views"  => $rowData[0][14],
                    ...
                    if ($this->db->insert("services",$data)) {
                        # code...
                        $data['error'] = FALSE;
                        $slugservices = $rowData[0][3];//$this->slug->create_uri($this->input->post('judul'));
                        //toxy categories
                        //$this->load->model('Tags_model','tags');
                        $datpost = $this->get_services_id($slugservices);
                        $idpost = $datpost[0]['id_services'];
    ...
    function kategori_services_edit($id){
        return $this->db->query("SELECT * FROM kategori where id_kategori='$id'");
    }

    function kategori_services_update(){
        $datadb = array('nama_kategori'=>$this->db->escape_str($this->input->post('a')),
                        'kategori_seo'=>seo_title($this->input->post('a')),
                        'aktif'=>$this->db->escape_str($this->input->post('b')));
    ...
    function kategori_services_delete($id){
    ...
    function tag_services_edit($id){
        return $this->db->query("SELECT * FROM tag where id_tag='$id'");
    }

    function tag_services_update(){
        $datadb = array('nama_tag'=>$this->db->escape_str($this->input->post('a')),
                        'tag_seo'=>seo_title($this->input->post('a')));
    ...
    function tag_services_delete($id){
        return $this->db->query("DELETE FROM tag where id_tag='$id'");
    }

    function komentar_services($id_services){
        return $this->db->query("SELECT * FROM komentar where id_services = '$id_services' AND aktif='Y'");
    }

    function kirim_komentar(){
        $datadb = array('id_services'=>cetak($this->input->post('a')),
    ...
    /*
    function list_services_rss(){
        return $this->db->query("SELECT a.*, b.nama_kategori FROM services a LEFT JOIN kategori b ON a.id_kategori=b.id_kategori ORDER BY a.id_services DESC LIMIT 10");
    }

    function list_services(){
        return $this->db->query("SELECT a.*, b.nama_kategori FROM services a LEFT JOIN kategori b ON a.id_kategori=b.id_kategori ORDER BY a.id_services DESC");
    }
    */
    function list_services_rss(){
        return $this->db->query("SELECT a.*, b.name FROM services a LEFT JOIN categories_lists b ON a.id_categories=b.id ORDER BY a.id_services DESC LIMIT 10");
    }

    function list_services(){
        return $this->db->query("SELECT a.*, b.name FROM services a LEFT JOIN categories_lists b ON a.id_categories=b.id ORDER BY a.id_services DESC");
    }

    
    function list_services_tambah(){
            $config['upload_path'] = '../asset/foto_services/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            ...
            $this->load->library('slug', $config);
            $slugservices = $this->slug->create_uri($this->input->post('judul'));
            // create the slug
            //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
            //fikar z
            ...
            if ($_FILES["gambar"]['name']==''){
                    $datadb = array('judul'=>$this->db->escape_str($this->input->post('judul')),
                                    ...
                                    'isi_services'=>$this->input->post('isi_services'),
        ...
        if ($this->db->insert('services',$datadb)){
            //if(file_exists('../asset/foto_services/'.$hasil['file_name'])){
                thumb('../asset/foto_services/'.$hasil['file_name'],'400','200');
            //}
        }
        //fikar tag toxi
        //$ci=& get_instance();
    ...
    function list_services_edit($id){
        return $this->db->query("SELECT * FROM services where id_services='$id'");
    }

    function list_services_update(){
        $config['upload_path'] = '../asset/foto_services/';
        $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
        $config['max_size'] = '3000'; // kb
        $new_name = substr(md5(uniqid(mt_rand())),0,10).'_'.time().'_'.$_FILES["gambar"]['name'];
        ...
        //fikar
            ...
            //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
            //fikar z
            $page = $this->db->where('id_services', $this->input->post('id'))->limit(1)->get('services')->row();
        ...
        if ($this->db->update('services',$datadb)){
            if ($imgupdate == true){
                //if(file_exists('../asset/foto_services/'.$hasil['file_name'])){
                    thumb('../asset/foto_services/'.$hasil['file_name'],'400','200');
                //}
    ...
    function list_services_delete($id){
        $page = $this->db->where('id_services', $id)->limit(1)->get('services')->row();
        //print_r($page);
        getnameimg('../asset/foto_services/'.$page->gambar,'foto_services');
    ...
    function update_services($data = array()){
        ...
        if (is_array($data) && ! empty($data))
        {
            ...
            $this->db->update_batch('services', $data, 'id_services');
        }
    }
}