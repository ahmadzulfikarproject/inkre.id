		<div class="row">
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="x_panel">
					...
					<div class="x_content slide">
						<div class="page-header">
							<!-- <h1><?php echo $title; ?> <small>Manage slide VIEW</small></h1> -->
							<?php if ($this->ion_auth->is_admin()) : ?>
								<a id="btn-add" class="btn btn-success pull-right" href="<?php echo site_url('slide/add_slide'); ?>"><i class="icon-plus icon-white"></i> Add slide</a>
							<?php endif; ?>
						</div>
						...
						<div class="info_box">
							<ol class="sortable ui-sortable">
								...
								if (!empty($posts)) : foreach ($posts as $post) : ?>
										...
										?>
										<li id="list_<?= $post['id_slide'];  ?>">
											<div>
												<i class="icon-move glyphicon glyphicon-move ui-sortable-handle"></i>
												<img src='<?php echo base_url() . "../asset/foto_slide/" . $gambar; ?>'>
												<?php echo $post['judul']; ?>
												<span class="pull-right">
													<a class="btn btn-primary btn-xs" href="<?php echo base_url('slide/edit_slide/') . $post['id_slide']; ?>"><span class="glyphicon glyphicon-pencil"></span> Edit</a> <a class="btn btn-danger btn-xs delete" data-toggle="modal" data-type="item" data-href="<?php echo base_url('slide/delete_slidebaru/') . $post['id_slide']; ?>" data-name="<?php echo $post['judul']; ?>" href="javascript:;"><span class="glyphicon glyphicon-trash"></span> Delete</a></span>
											</div>
										</li>
									<?php $no++;
									endforeach;
		...
		<!-- <script>
			$(document).ready(function() {
				...
				$('#featured-slide').sortable({
					axis: 'y',
					stop: function(event, ui) {
						var data = $(this).sortable('serialize');
						//$('#featured-slide-sort').text(data);
						$.ajax({
							type: 'post',
							url: '<?php echo base_url(); ?>slide/reorder_slide',
							data: {
								order: data,
	            ...
	            */
				...
				$(".remove").click(function(){
				    var id = $(this).parents(".itemslide").attr("id");
		...
		<script>
			...
			var LIST_MAX_LEVELS = "<?php echo $this->config->item('max_levels', 'slide_list'); ?>";
		</script>
		<!--nested menu end-->
		<?php
		// ob_end_flush();
