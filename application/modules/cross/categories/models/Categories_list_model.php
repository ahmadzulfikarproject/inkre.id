 ...
 * @since   Version 0.1
 */
class Categories_list_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		$this->load->database();
		$this->load->config('categories_list', TRUE);
		$this->lang->load('categories_list');

		$this->tables = $this->config->item('tables', 'categories_list');
		$this->flash  = $this->config->item('flash', 'categories_list');
	}
	...
	public function get_all_front($group)
	{
		if ( ! is_numeric($group))
		{
			$group = $this->db->where('slug', $group)->get($this->tables['groups'])->row()->id;
...
/* End of file categories_list_model.php */
/* Location: ./models/categories_list_model.php */
