...
if ( ! function_exists('build_admin_tree_cat'))
{
	/**
	 * Build admin tree
	 *
	 * Creates categories list for administration
	 *
	 * @param array &$tree Tree items
	 *
	 * @return string
	 */
	function build_admin_tree_cat(&$tree)
	{
		$output = '';
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				...
				{
					$output .= '<li id="list_' . $leaf['id'] . '"><div><i class="icon-move glyphicon glyphicon-move"></i> ' . $leaf['name'] . '<span class="pull-right"><a class="btn btn-primary btn-xs" href="' . site_url('categories/edit/' . $leaf['id']) . '"><span class="glyphicon glyphicon-pencil"></span> Edit</a> <a class="btn btn-danger btn-xs delete" data-toggle="modal" data-type="item" data-href="' . site_url('categories/delete/' . $leaf['id']) . '" data-name="' . $leaf['name'] . '" href="javascript:;"><span class="glyphicon glyphicon-trash"></span> Delete</a></span></div>';
					$output .= '<ol>' . build_admin_tree_cat($leaf['children']) . '</ol>';
					$output .= '</li>';
				}
				else
				{
...
if ( ! function_exists('build_tree'))
{
	 ...
	 * Build tree
	 *
	 * Creates categories list based on group id or slug
	 *
	...
	function build_tree($group, $attributes = array(), &$tree = array())
	{
		...
		{
			$CI =& get_instance();
			$CI->load->library('categories_list');

			$tree = $CI->categories_list->get_all($group);
...
if ( ! function_exists('build_categories_sidebar'))
{
	/**
	 * Build tree
	 *
	 ...
	 * @return string
	 */
	function classcategories($classmenu) {
		$CI = get_instance();
		$slug = $classmenu;
		$segment = $CI->uri->uri_string();
		/*
	...
	function carianakcategories($anaks,$segment,$group = 'products'){
						...
						foreach ($anaks as $key => $anak) {
							...
							//echo $anak['slug'];
							//echo "<br>";
							if(preg_match("/^http/", $anak['slug'])) {
							$anak['slug'] = $anak['slug'];
		                    //echo "<li><a href='$row[link_sub]'>$row[nama_sub]</a></li>";
			                  }else{
			                    //echo "<li><a href='".base_url()."$row[link_sub]'>$row[nama_sub]</a></li>";
			                    $anak['slug'] = base_url($group.'/categories/'.$anak['slug']); //base_url().$anak['slug'];
			                  }
							if ($anak['slug'] == $segment){
	...
	function build_categories_sidebar($group, $attributes = array(), &$tree = array())
	{
		$CI = get_instance();
		...
		if (empty($tree))
		{
			...
			$tree = $CI->categories_list->get_all($group);
			$tree = parse_children_categories($tree);
		}
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				...
				if (isset($leaf['children']) && ! empty($leaf['children']))
				{
					...
					/*
					$output .= $atts['start_tag'] . '<i class="icon-move glyphicon glyphicon-plus ui-sortable-handle"></i><a href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
					$output .= $atts['sub_start_tag'] . build_categories_sidebar($group, $attributes, $leaf['children']) . $atts['sub_end_tag'];
					$output .= $atts['end_tag'];
					*/
					//fikar
					//classcategories($leaf['slug']);
	                //echo "<br>";
	                $linkku = $leaf['slug'];
					if(preg_match("/^http/", $leaf['slug'])) {
						$leaf['slug'] = $leaf['slug'];
	                    //echo "<li><a href='$row[link_sub]'>$row[nama_sub]</a></li>";
	                  }else{
	                    //echo "<li><a href='".base_url()."$row[link_sub]'>$row[nama_sub]</a></li>";
	                    $leaf['slug'] = base_url($group.'/categories/'.$leaf['slug']); //base_url().$leaf['slug'];
	                  }
	                $segment = $CI->uri->uri_string();
	                  if(preg_match("/^http/", $segment)) {
						$segment = $segment;
	                    ...
	                    //echo "<li><a href='".base_url()."$row[link_sub]'>$row[nama_sub]</a></li>";
	                    $segment = base_url($segment); //base_url().$segment;
	                  }
	                //cari anak
	                //print_r($leaf['children']);
					//echo "<hr>";
					...
					//carianakcategories($leaf['children'],$segment);
					
					//cari anak end
	                if ($segment == $leaf['slug']){
						//$output .= '<li class="item-1 deeper parent active ">'. '<a data-toggle="dropdown" class="dropdown-toggle" href="#">'.$leaf['name'].' <b class="caret"></b></a><a class="hidden" href="' . $leaf['slug'] . '"><h4 class="lbl">'.$leaf['name'].'</h4></a>';
						$active_link = array();
								$active_link['active'] = "active";
								$active_link['in'] = "in";
						$output .= '<li class="item-1 deeper parent active">' . '<a class="" href="'.$leaf['slug'].'"><span data-toggle="collapse" data-parent="#menu-group-1" href="#'.$linkku.'" class="sign"><i class="fa fa-plus-circle icon-white"></i></span><h4 class="lbl">'.$leaf['name'].'</h4></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
					...
					elseif ($linkku == $CI->router->fetch_class()){
						$active_link = array();
								$active_link['active'] = "active";
								$active_link['in'] = "in";
						$output .= '<li class="item-1 deeper parent active">' . '<a class="" href="#">'.$leaf['name'].' <span data-toggle="collapse" data-parent="#menu-group-1" href="#sub-item-1" class="sign"><i class="fa fa-plus-circle icon-white"></i></span></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';

					}
					else{
						//echo ($segment == 'settings-seo' || $segment == 'settings-email') ? 'class="active"' : 'class=""';
						$active_link = carianakcategories($leaf['children'],$segment,$group);
						$output .= '<li class="item-1 deeper parent '.$active_link['active'].'">' . '<a class="" href="'.$leaf['slug'].'"><span data-toggle="collapse" data-parent="#menu-group-1" href="#'.$linkku.'" class="sign"><i class="fa fa-plus-circle icon-white"></i></span><h4 class="lbl">'.$leaf['name'].'</h4></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
						//$output .= $atts['parent_tag'] . '<a data-toggle="dropdown" class="dropdown-toggle" href="#">'.$leaf['name'].' <b class="caret"></b></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
					}
					
					//$output .= $atts['sub_start_tag'] . build_categories_sidebar($group, $attributes, $leaf['children']) . $atts['sub_end_tag'];
					//$output .= $atts['parent_end_tag'];

					$output .= '<ul class="children nav-child list-unstyled small collapse '.$active_link['in'].'" id="'.$linkku.'">' . build_categories_sidebar($group, $attributes, $leaf['children']) .'</ul>';
					$output .= $atts['parent_end_tag'];

					
					//$atts['start_tag'] .= set_active($atts['start_tag'], $leaf['slug']);
				...
				else
				{
					...
					if ($segment == $leaf['slug']){
						$output .= '<li class="active list-group-item">'. '<a href="' . $leaf['slug'] . '"><h4 class="lbl">' . $leaf['name'] . '</h4></a>' . $atts['end_tag'];
					}
					...
					elseif ($linkasli == $CI->router->fetch_class()){
						...
						$output .= $atts['start_tag'] . '<a href="' . $leaf['slug'] . '"><h4 class="lbl">' . $leaf['name'] . '</h4></a>' . $atts['end_tag'];
					}			
	                  //echo '<li class="okehhh" >okeh</li>';
	...
	function build_categories_sidebar2($group, $attributes = array(), &$tree = array())
	{
		$CI = get_instance();
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				...
				if (isset($leaf['children']) && ! empty($leaf['children']))
				{
					...
					*/
					...
					if(preg_match("/^http/", $leaf['slug'])) {
	                    ...
	                    //echo "<li><a href='".base_url()."$row[link_sub]'>$row[nama_sub]</a></li>";
	                    $leaf['slug'] = base_url('products/categories/'.$leaf['slug']); //base_url().$leaf['slug'];
	                  }
	                $segment = $CI->uri->uri_string();
	                  if(preg_match("/^http/", $segment)) {
						$segment = $segment;
	                    ...
	                    //echo "<li><a href='".base_url()."$row[link_sub]'>$row[nama_sub]</a></li>";
	                    $segment = base_url('products/categories/'.$segment); //base_url().$segment;
	                  }
	                //cari anak
	                //print_r($leaf['children']);
					//echo "<hr>";
					...
					//carianakcategories($leaf['children'],$segment);
					$active_link = carianakcategories($leaf['children'],$segment);
					//cari anak end
	                if ($segment == $leaf['slug']){
						$output .= '<li class="parentku panel panel-default active ">'. '<a data-toggle="dropdown" class="dropdown-toggle" href="#">'.$leaf['name'].' <b class="caret"></b></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
					}
					elseif ($linkku == $CI->router->fetch_class()){
						$output .= '<li class="parentku panel panel-default active">' . '<a class="" href="#">'.$leaf['name'].' <span data-toggle="collapse" data-parent="#menu-group-1" href="#sub-item-1" class="sign"><i class="fa fa-plus-circle icon-white"></i></span></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
					}
					else{
						//echo ($segment == 'settings-seo' || $segment == 'settings-email') ? 'class="active"' : 'class=""';

						////$output .= '<li class="panel panel-default '.$active_link.'">' . '<a class="" href="#"><span data-toggle="collapse" data-parent="#menu-group-1" href="#'.$linkku.'" class="sign"><i class="fa fa-plus-circle icon-white"></i></span><span class="lbl">'.$leaf['name'].'</span></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
						//$output .= $atts['parent_tag'] . '<a data-toggle="dropdown" class="dropdown-toggle" href="#">'.$leaf['name'].' <b class="caret"></b></a><a class="hidden" href="' . $leaf['slug'] . '">' . $leaf['name'] . '</a>';
						$output .= '<li class="panel panel-default '.$active_link.'">';
						$output .= '<div class="panel-heading">
                						<h4 class="panel-title">
                    						<a data-toggle="collapse" data-parent="#accordion" href="#'.$linkku.'2"><span class="glyphicon glyphicon-plus"></span> '.$leaf['name'].'</a>
                						</h4>
            						</div>';
            			$output .= '<div id="'.$linkku.'2" class="panel-collapse collapse">
					...
					$output .= '<ul class="list-unstyled collapsez" id="'.$linkku.'2">' . build_categories_sidebar($group, $attributes, $leaf['children']) .'</ul>';
					$output .= $atts['parent_end_tag'];
				...
				else
				{
					...
					if ($segment == $leaf['slug']){
						$output .= '<li class="active">'. '<a href="' . $leaf['slug'] . '"><span class="lbl">' . $leaf['name'] . '</span></a>' . $atts['end_tag'];
					}
					...
					elseif ($linkasli == $CI->router->fetch_class()){
						...
						$output .= $atts['start_tag'] . '<a href="' . $leaf['slug'] . '"><span class="lbl">' . $leaf['name'] . '</span></a>' . $atts['end_tag'];
					}			
	                  //echo '<li class="okehhh" >okeh</li>';
...
if ( ! function_exists('build_tree_item'))
{
	 ...
	 * Build tree item
	 *
	 * Creates categories list based on group (id or slug) and shows leafs related only to current item
	 *
	 * @param mixed $group        Group id or slug
	 * @param int   $item_id      Current item id
	 * @param array $attributes   Any attributes
...
if ( ! function_exists('parse_children_categories'))
{
	/**
	 * Parse children
	 *
	 ...
	 * @return array|bool
	 */
	function parse_children_categories(&$query)
	{
		! empty($query) OR FALSE;
...
/* Location: ./helpers/categories_list_helper.php */
