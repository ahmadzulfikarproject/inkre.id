...
class Projects_model extends CI_Model
{
    ...
    var $table = 'projects';
    var $column_order = array(null,null, 'judul', 'cat_name','projects_views','created_time',null); //set column field database for datatable orderable
    var $column_search = array('judul', 'id_categories'); //set column field database for datatable searchable 
    var $order = array('id_projects' => 'desc'); // default order 
    var $hasil = array();
    var $wm = false;
    var $new_slug = false;
    var $wide = 400;
    ...
    private function _get_datatables_query()
    {
        ...
        // $this->db->select('projects.*, categories_lists.slug AS cat_slug,categories_lists.id');
        $this->db->select('projects.*, categories_lists.slug AS cat_slug,categories_lists.id,categories_lists.name AS cat_name');
    ...
    function projects_add()
    {
        ...
        $datadb = array(
            ...
            'isi_projects' => $this->input->post('b'),
        ...
        if ($this->db->insert($this->table, $datadb)) {
            ...
            if (($datadb['gambar']) && (file_exists('../asset/foto_projects/' . $datadb['gambar']))) {
                thumb('../asset/foto_projects/' . $datadb['gambar'], $this->wide, $this->height, $this->wm);
            }
        }
    }
    function projects_edit($id)
    {
        return $this->db->query("SELECT * FROM $this->table where id_projects='$id'");
    }
    function projects_update()
    {
        ...
        //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
        //fikar z
        $page = $this->db->where('id_projects', $this->input->post('id'))->limit(1)->get($this->table)->row();
    ...
    function projects_delete($id)
    {
        ...
        $projects = $this->db->where('id_projects', $id)->limit(1)->get($this->table)->row();
        //print_r($projects);
        getnameimg('../asset/foto_projects/' . $projects->gambar, 'foto_projects');
        //unlink('../asset/foto_projects/'.$projects->gambar);
        return $this->db->query("DELETE FROM $this->table where id_projects='$id'");
    }
    //toxi tag
    ...
    function reset_db_projects()
    {
        ...
        foreach (glob('../asset/foto_projects/*') as $rowimg) {
