...
class Model_gallery extends CI_model
{
            ...
            if ($media['file_ext'] == '.csv') {
                ...
                foreach ($file_data as $row) {
                    ...
                    if ($ssrc != '') {
                        ...
                        if (!is_dir("../asset/foto_gallery/")) {
                            mkdir("../asset/foto_gallery/", 0777, TRUE);
                        }
                        //upload gambar
                        /*
                        unset($config);
    ...
    {
        $datadb = array(
            'id_gallery' => cetak($this->input->post('a')),
    ...
    */
    ...
    function list_gallery_tambah()
    {
        ...
        if ($this->db->insert('gallery', $datadb)) {
            ...
            thumb('../asset/foto_gallery/' . $hasil['file_name'], '400', '200', $wm);
            //}
            $this->list_gallery_uploadimg($this->db->insert_id(),'gallery');
        }
    ...
    function list_gallery_uploadimg($id, $type = 'gallery')
    {

        /*
        $datalama = $this->getRows_gallery($this->input->post('id'));
        $folderimg = '../asset/foto_gallery/gallery/';
        //print_r($datalama);
        ...
        */
        ...
        if (!empty($_FILES['pro-image']['name'])) {
            ...
            for ($i = 0; $i < $filesCount; $i++) {
                ...
                $config['upload_path'] = '../asset/foto_gallery/gallery/';
                $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
                $config['max_size'] = '3000'; // kb
                ...
                if ($this->upload->do_upload('file')) {
                    ...
                    thumb('../asset/foto_gallery/gallery/' . $fileData['file_name'], '400', '200', $wm);
                    $uploadData[$i]['id_post'] = $id;
            ...
            if (!empty($uploadData)) {
                $this->list_gallery_deleteimg($id, $type);
                //print_r($uploadData);
                // echo implode(",",$uploadData);
                // Insert files data into the database

                $insert = $this->insert_gallery_db($uploadData);
    ...
    function list_gallery_deleteimg($id, $type = 'gallery')
    {
        $datalama = $this->getRows_gallery($this->input->post('id'), $type);
        $folderimg = '../asset/foto_gallery/gallery/';
        //print_r($datalama);
        if (!empty($datalama)) {
            foreach ($datalama as $file) {
    ...
    function insert_gallery_db($data = array())
    {
        $insert = $this->db->insert_batch('files', $data);
        return $insert ? true : false;
    }
    function getRows_gallery($id = '', $post_type = 'gallery')
    {
        $this->db->select('*');
        $this->db->from('files');
        if ($id) {
    ...
    {
        $this->list_gallery_deleteimg($id, 'gallery');
