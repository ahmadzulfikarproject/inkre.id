...
class Clients_model extends CI_Model
{
    ...
    var $table = 'clients';
    var $column_order = array(null,null, 'judul', 'cat_name','clients_views','created_time',null); //set column field database for datatable orderable
    var $column_search = array('judul', 'id_categories'); //set column field database for datatable searchable 
    var $order = array('id_clients' => 'desc'); // default order 
    var $hasil = array();
    var $wm = false;
    var $new_slug = false;
    var $wide = 400;
    ...
    private function _get_datatables_query()
    {
        ...
        // $this->db->select('clients.*, categories_lists.slug AS cat_slug,categories_lists.id');
        $this->db->select('clients.*, categories_lists.slug AS cat_slug,categories_lists.id,categories_lists.name AS cat_name');
    ...
    function clients_add()
    {
        ...
        if ($this->db->insert($this->table, $datadb)) {
            ...
            if (($datadb['gambar']) && (file_exists('../asset/foto_clients/' . $datadb['gambar']))) {
                thumb('../asset/foto_clients/' . $datadb['gambar'], $this->wide, $this->height, $this->wm);
            }
        }
    }
    function clients_edit($id)
    {
        return $this->db->query("SELECT * FROM $this->table where id_clients='$id'");
    }
    function clients_update()
    {
        ...
        //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
        //fikar z
        $page = $this->db->where('id_clients', $this->input->post('id'))->limit(1)->get($this->table)->row();
    ...
    function clients_delete($id)
    {
        ...
        $clients = $this->db->where('id_clients', $id)->limit(1)->get($this->table)->row();
        //print_r($clients);
        getnameimg('../asset/foto_clients/' . $clients->gambar, 'foto_clients');
        //unlink('../asset/foto_clients/'.$clients->gambar);
        return $this->db->query("DELETE FROM $this->table where id_clients='$id'");
    }
    //toxi tag
    ...
    function reset_db_clients()
    {
        ...
        foreach (glob('../asset/foto_clients/*') as $rowimg) {
