...
class products_model extends CI_Model
{
    ...
    var $table = 'products';
    var $column_order = array(null,null, 'judul', 'cat_name','products_views','created_time',null); //set column field database for datatable orderable
    var $column_search = array('judul', 'id_categories'); //set column field database for datatable searchable 
    var $order = array('id_products' => 'desc'); // default order 
    var $hasil = array();
    var $wm = false;
    var $new_slug = false;
    var $wide = 400;
    ...
    private function _get_datatables_query()
    {
        ...
        // $this->db->select('products.*, categories_lists.slug AS cat_slug,categories_lists.id');
        $this->db->select('products.*, categories_lists.slug AS cat_slug,categories_lists.id,categories_lists.name AS cat_name');
    ...
    function get_all_categories($id = '', $group_id = '5')
    {
        $this->db->select('*');
    ...
    function products_add()
    {
        ...
        $datadb = array(
            ...
            'isi_products' => $this->input->post('b'),
        ...
        if ($this->db->insert($this->table, $datadb)) {
            ...
            if (($datadb['gambar']) && (file_exists('../asset/foto_products/' . $datadb['gambar']))) {
                thumb('../asset/foto_products/' . $datadb['gambar'], $this->wide, $this->height, $this->wm);
            }
        }
    }
    function products_edit($id)
    {
        return $this->db->query("SELECT * FROM $this->table where id_products='$id'");
    }
    function products_update()
    {
        ...
        //$datadb['url_title'] = $this->slug->create_uri($post_data['title']);
        //fikar z
        $page = $this->db->where('id_products', $this->input->post('id'))->limit(1)->get($this->table)->row();
    ...
    function products_delete($id)
    {
        ...
        $products = $this->db->where('id_products', $id)->limit(1)->get($this->table)->row();
        //print_r($products);
        getnameimg('../asset/foto_products/' . $products->gambar, 'foto_products');
        //unlink('../asset/foto_products/'.$products->gambar);
        return $this->db->query("DELETE FROM $this->table where id_products='$id'");
    }
    //toxi tag
