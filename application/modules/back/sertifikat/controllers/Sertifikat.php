...
class Sertifikat extends MY_AdminController {
	...
	function index2(){
        ...
        //$this->load->view(template().'/sertifikat-rss',$data);
        //$this->fcore->set_meta($data['rss'], 'sertifikat');
        //$this->template->loadrss(template().'/sertifikat-rss',$data);
		$this->template->load('administrator/template','view_sertifikat',$data);
	}
	function index(){
		cek_session_admin();
	...
	function cepat_sertifikat(){
		cek_session_admin();
		if (isset($_POST['submit'])){
			$this->model_sertifikat->list_sertifikat_cepat();
			redirect('sertifikat');
		}
	} 
	...
	function tambah_sertifikat(){
		...
		if (isset($_POST['submit'])){
			...
			$this->model_sertifikat->list_sertifikat_tambah();
		...
		elseif (isset($_POST['savenew'])) {
			...
			$this->model_sertifikat->list_sertifikat_tambah();
			//$this->model_sertifikat_tags->insert_sertifikat_tagsdb($tags);
			redirect('tambah_sertifikat');
		}
		else{
			$data['tag'] = $this->model_sertifikat->tag_sertifikat();
			$data['record'] = $this->model_sertifikat->categories_sertifikat();
			$data['users'] = $this->model_users->users();
			$this->template->load('administrator/template','view_sertifikat_tambah',$data);
		}
	}
	...
	function edit_sertifikat(){
		...
		if (isset($_POST['submit'])){
			...
			//$tags = $this->db->escape_str($this->input->post('tagshasil'));
			$this->model_sertifikat->list_sertifikat_update();
			//$this->model_sertifikat_tags->insert_sertifikat_tagsdb($tags);
            ...
            if (!empty($tags)) {
    			...
    			$this->model_sertifikat_tags->insert_sertifikat_tagsmap($tags,$post_id,'sertifikat');
            }
		...
		elseif (isset($_POST['update'])) {
			...
			redirect('sertifikat/edit_sertifikat/'.$_POST['id']);
		}
		else{
			$post_id = $this->db->escape_str($this->input->post('id'));
			//echo $post_id;
			$data['tags'] = $this->model_sertifikat->tags_sertifikat($id);

			$data['tag'] = $this->model_sertifikat->tag_sertifikat();
			$data['record'] = $this->model_sertifikat->categories_sertifikat();
			$data['rows'] = $this->model_sertifikat->list_sertifikat_edit($id)->row_array();
			$data['users'] = $this->model_users->users();

			//print_r($data['tags']->row_array());
			$this->template->load('administrator/template','view_sertifikat_edit',$data);
		}
	}

	function delete_sertifikat(){
		$id = $this->uri->segment(3);
		$this->model_sertifikat->list_sertifikat_delete($id);
		redirect('sertifikat');
	}
	function update_semua_sertifikat(){
		cek_session_admin();
		$config = array(
                'field' => 'slug',
                'title' => 'judul',
	    ...
	    $results = $this->model_sertifikat->list_sertifikat();
	    ...
	    foreach ($results->result_array() as $key => $value)
	    {
	        ...
	        $data[$key]['id_sertifikat']    = $value['id_sertifikat'];
	    ...
	    // Update database
	    $this->model_sertifikat->update_sertifikat($data);
		//$data['record'] = $this->model_sertifikat->semua_sertifikatterbaru();
		
		 //$this->model_sertifikat->semua_sertifikat();
		//print_r($data);
		//$this->model_sertifikat->update_semua_sertifikat();
		redirect('sertifikat');
	}
    //fikar custom
    function import(){
    ...
    function do_upload(){
      if ($this->input->is_ajax_request()){
        $this->model_sertifikat->upload_data();
      }else{
        show_404();
      }
    }
    function get_autocomplete(){
        if ($this->input->is_ajax_request())
        {
            ...
            if (isset($_GET['term'])) {
                $result = $this->model_sertifikat_tags->search_sertifikat_tags($_GET['term']);
                if (count($result) > 0) {
    ...
    function reset_sertifikat(){
      if ($this->input->is_ajax_request()){
        $this->model_sertifikat->reset_db_sertifikat();
      }else{
        show_404();
      }
    }
